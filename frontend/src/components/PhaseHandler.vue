...
async function modulate() {
    ...
    const res = await axios.post<{ path: string }>('/phase-modulate', {
...
async function demodulate() {
    ...
    const res = await axios.post<{ data: string; isBinary: boolean }>('/phase-demodulate', {
        frequency: store.frequency,
        sampleRate: store.sampleRate,
